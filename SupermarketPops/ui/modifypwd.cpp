...
{
    QSqlQuery query;
    QString queryselect = QString(" select password from user where staffId = '%1';").arg(LoginDialog::role);
        ...
        else
        {
            if(ui->newPwdLineEdit->text() == ui->confirmPwdLineEdit->text())
            {
                ...
                QString sql = QString("update user set password = '%1' where staffId = '%2'").arg(newPassword,LoginDialog::role);
